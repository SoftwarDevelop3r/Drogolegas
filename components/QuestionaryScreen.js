...
export function QuestionaryScreen({ navigation }) {
  ...
  const renderAskAndResponses = ({ item }) => (
    <View key={item.ask}>
      <Question question_number={item.ask} question={item.text} />
      <Answer
        answer_number={item.ask}
        answer_possibilities={item.possibilities}
  ...
  return (
    <FlatList
      data={questions}
      renderItem={renderAskAndResponses}
      keyExtractor={item => item.ask}
      ListHeaderComponent={() => (
        <Information
          message='El siguiente cuestionario calcula qué tan drogolega eres. Por favor, una vez obtenidos tus resultados, compártelos con los demás en el grupo de WhatsApp.'
        />
      )}
      ListFooterComponent={() => (
